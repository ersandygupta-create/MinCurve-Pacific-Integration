page 50183 "E3 LIS GRN List"
{

    ApplicationArea = All;
    Caption = 'LIS GRN List';
    PageType = List;
    Editable = false;
    CardPageId = "E3 HIS GRN Header";
    InsertAllowed = false;
    SourceTable = "E3 HIS Purchase Header";
    SourceTableView = sorting("Entry No.") where("Record Type" = Filter(GRN), "Document Type" = filter(Order), "Capex Type" = filter('' | Opex), "Create PO" = filter(false), Source = filter('ITDOSE'));
    UsageCategory = Lists;
