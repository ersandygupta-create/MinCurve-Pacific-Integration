report 50048 "E3 Create Purchase Order"
{
    ProcessingOnly = true;
    UsageCategory = Tasks;
    ...
    dataset
    {
        dataitem("E3 Purchase Indent Line"; "E3 Purchase Indent Line")
        {
            ...
            begin
                if "E3 Purchase Indent Line"."Requested Qty" = 0 then
                    "E3 Purchase Indent Line".FieldError("Requested Qty");

                if ("E3 Purchase Indent Line"."Vendor No." <> '') and ("E3 Purchase Indent Line"."Vendor PO Creation" = true) then begin

                    // Calculate Remaining Qty
                    RemainingQty := GetRemainingQty("E3 Purchase Indent Line");

                    // Ordered Qty cannot exceed Remaining Qty
                    if "E3 Purchase Indent Line"."Ordered Qty" > RemainingQty then
                        Error(
                            'Ordered Qty (%1) cannot be greater than Remaining Approved Qty (%2).',
                            "E3 Purchase Indent Line"."Ordered Qty",
                            RemainingQty);

                    if LastSupplier <> "E3 Purchase Indent Line"."Vendor No." then
                        CreatePurchaseHeader("E3 Purchase Indent Line", 1);

                    CreatePurchaseLines("E3 Purchase Indent Line", 1);

                    // Update Remaining Qty
                    "E3 Purchase Indent Line"."Created PO Qty" += PurchaseLine.Quantity;
                    "E3 Purchase Indent Line"."Purchase Order No." := PurchaseLine."Document No.";
                    "E3 Purchase Indent Line"."PO Created" := true;
                    "E3 Purchase Indent Line".Modify();
                end;

                ProcessingCompleted("E3 Purchase Indent Line");
            end;

            trigger OnPostDataItem()
            var
                IndentLineRec: Record "E3 Purchase Indent Line";
                IndentHeaderRec: Record "E3 Purchase Indent Header";
            begin
                DialogWindow.Close();

                // Update Release Indent based on remaining quantity
                IndentHeaderRec.Reset();
                IndentHeaderRec.SetRange("Document No.", "E3 Purchase Indent Line"."Document No.");
    ...
    local procedure ProcessingCompleted(IndentLine: Record "E3 Purchase Indent Line")
    begin
    ...
    local procedure CreatePurchaseHeader(IndentLine: Record "E3 Purchase Indent Line"; PurchHeaderType: Integer)
    var
        RequistionHeader: Record "E3 Purchase Indent Header";
    ...
    local procedure InsertIndentLineDetails(IndentLine: Record "E3 Purchase Indent Line")
    var
    ...
    local procedure GetRemainingQty(IndentLine: Record "E3 Purchase Indent Line"): Decimal
    var
    ...
    local procedure GetOrderedQty(IndentLine: Record "E3 Purchase Indent Line"): Decimal
    var
        PurchLine: Record "Purchase Line";
        TotalOrderedQty: Decimal;
    begin
