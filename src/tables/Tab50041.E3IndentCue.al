table 50041 "E3 Indent Cue"
{
    ...
    fields
    {
        ...
        field(2; "Open Indents"; Integer)
        {
            ...
            CalcFormula = Count("E3 Purchase Indent Header" where(Status = const(Open)));
            Caption = 'Open Indents';
        }
        field(3; "Pending Approval"; Integer)
        {
            FieldClass = FlowField;
            CalcFormula = Count("E3 Purchase Indent Header" where(Status = const("Pending Approval")));
            Caption = 'Pending Approval';
        }
        field(4; "Approved Indents"; Integer)
        {
            FieldClass = FlowField;
            CalcFormula = Count("E3 Purchase Indent Header" where(Status = const(Approved)));
            Caption = 'Approved Indents';
        }
        field(5; "Rejected Indents"; Integer)
        {
            FieldClass = FlowField;
            CalcFormula = Count("E3 Purchase Indent Header" where(Status = const(Rejected)));
