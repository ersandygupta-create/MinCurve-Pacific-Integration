codeunit 50028 "E3 ITDose Integration Mgmt."
{
    ...
    begin
        ...
        case Rec."Parameter String" of
            'Supplier', 'supplier', 'SUPPLIER':
                if E3AkhilSetup."LIS Vendor Master API Enabled" then
                    SyncSupplier(Rec);
    ...
    var
        E3SupplierLog: Record "E3 API Suppl Update Log ITDose";
    begin
        // Step 1: Create Log
    ...
    begin
        E3AkhilSetup.get();
        E3AkhilSetup.TestField("LIS Username");
        E3AkhilSetup.TestField("LIS Password");

        BasicCred := E3AkhilSetup."LIS Username" + ':' + E3AkhilSetup."LIS Password";
    ...
    begin
        ...
        if not E3AkhilSetup."LIS Vendor Master API Enabled" then
            exit;
    ...
    var
        E3SupplierLog: Record "E3 API Suppl Update Log ITDose";
        xE3SupplierLog: Record "E3 API Suppl Update Log ITDose";
    begin
    ...
    procedure EnqueueVendorJobEntry(APIVendorUpdateLog: Record "E3 API Suppl Update Log ITDose"): Guid
    var
    ...
    var
        SupplierUpdateLog: Record "E3 API Suppl Update Log ITDose";
        xSupplierUpdateLog: Record "E3 API Suppl Update Log ITDose";
    begin
        if not E3AkhilSetup."LIS Vendor Master API Enabled" then
            exit;
    ...
    procedure SendSupplierDetails(var SupplierUpdateLog: Record "E3 API Suppl Update Log ITDose"): Boolean
    var
    ...
    begin
        ...
        JChildObj.Add(
            'CreationDate',
            Format(DT2Date(SupplierUpdateLog."Last Modified Date Time"), 0, '<Day,2>-<Month,2>-<Year4>')
        );

        JChildObj.Add(
            'CreationTime',
            Format(DT2Time(SupplierUpdateLog."Last Modified Date Time"))
        );
        ...
        RequestMessage.SetRequestUri(E3AkhilSetup."LIS Vendor Master API");
        ...
        if JObject.SelectToken('supplierMasterStatus', JToken) then begin
            ...
            if JToken.IsArray then begin
                ...
                for J := 0 to JArray.Count - 1 do begin
                    ...
                    JArray.Get(J, CJToken);

                    if CJToken.IsObject then begin

                        JObject := CJToken.AsObject();
                        ...
                        // ---------------- SUCCESS CHECK (FIXED) ----------------
                        if StrPos(
                            UpperCase(DelChr(IsSuccess, '=', ' ')),
                            UpperCase('CREATEDSUCCESSFUL')
                        ) > 0 then begin
                            ...
                            SupplierUpdateLog."Sync Status" :=
                                SupplierUpdateLog."Sync Status"::Error;

                            SupplierUpdateLog."Error Message" :=
                                CopyStr(IsSuccess, 1, 250);
                ...
                SupplierUpdateLog."Sync Status" := SupplierUpdateLog."Sync Status"::Error;
                SupplierUpdateLog."Error Message" := 'supplierMasterStatus is not an array';
                SupplierUpdateLog.Modify();
                exit(false);
            end;
            ...
            SupplierUpdateLog."Sync Status" := SupplierUpdateLog."Sync Status"::Error;
            SupplierUpdateLog."Error Message" := 'supplierMasterStatus node not found';
            SupplierUpdateLog.Modify();
            exit(false);
        end;
